import { myHttpGet } from './http.js'

const apiEndpoint = 'http://localhost:3000/' //Serveur doit être lancé en local:8080
...
export async function apiGetRateHistory (ticker) {
  ...
  const rawResponse = (await myHttpGet(`${apiEndpoint}${ticker}-rates-5y.json`)).data
